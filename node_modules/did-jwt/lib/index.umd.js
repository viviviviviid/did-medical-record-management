(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('uint8arrays'), require('multiformats/basics'), require('@noble/curves/ed25519'), require('@noble/hashes/sha256'), require('@noble/hashes/ripemd160'), require('@noble/hashes/sha3'), require('uint8arrays/from-string'), require('uint8arrays/to-string'), require('uint8arrays/concat'), require('@noble/curves/secp256k1'), require('@noble/curves/p256'), require('canonicalize'), require('did-resolver'), require('bech32'), require('@stablelib/xchacha20poly1305'), require('@noble/hashes/utils')) :
  typeof define === 'function' && define.amd ? define(['exports', 'uint8arrays', 'multiformats/basics', '@noble/curves/ed25519', '@noble/hashes/sha256', '@noble/hashes/ripemd160', '@noble/hashes/sha3', 'uint8arrays/from-string', 'uint8arrays/to-string', 'uint8arrays/concat', '@noble/curves/secp256k1', '@noble/curves/p256', 'canonicalize', 'did-resolver', 'bech32', '@stablelib/xchacha20poly1305', '@noble/hashes/utils'], factory) :
  (global = global || self, factory(global.didJwt = {}, global.uint8Arrays, global.basics, global.ed25519, global.sha256$1, global.ripemd160, global.sha3, global.fromString, global.toString, global.concat, global.secp256k1, global.p256, global.canonicalize, global.didResolver, global.bech32, global.xchacha20poly1305, global.utils));
}(this, (function (exports, uint8arrays, basics, ed25519, sha256$1, ripemd160, sha3, fromString, toString, concat, secp256k1, p256, canonicalizeData, didResolver, bech32, xchacha20poly1305, utils) {
  ...
  function sha256(payload) {
    const data = typeof payload === 'string' ? fromString.fromString(payload) : payload;
    return sha256$1.sha256(data);
  }
  const keccak = sha3.keccak_256;
  function toEthereumAddress(hexPublicKey) {
    const hashInput = fromString.fromString(hexPublicKey.slice(2), 'base16');
    return `0x${toString.toString(keccak(hashInput).slice(-20), 'base16')}`;
  }

  function writeUint32BE(value, array = new Uint8Array(4)) {
    const encoded = fromString.fromString(value.toString(), 'base10');
    array.set(encoded, 4 - encoded.length);
    return array;
  }

  const lengthAndInput = input => concat.concat([writeUint32BE(input.length), input]); // This implementation of concatKDF was inspired by these two implementations:
  ...
  function concatKDF(secret, keyLen, alg, producerInfo, consumerInfo) {
    ...
    const value = concat.concat([lengthAndInput(fromString.fromString(alg)), lengthAndInput(typeof producerInfo === 'undefined' ? new Uint8Array(0) : producerInfo), lengthAndInput(typeof consumerInfo === 'undefined' ? new Uint8Array(0) : consumerInfo), writeUint32BE(keyLen)]); // since our key lenght is 256 we only have to do one round

    const roundNumber = 1;
    return sha256(concat.concat([writeUint32BE(roundNumber), secret, value]));
  }
  ...
  const decryptJWE = function (jwe, decrypter) {
    try {
      ...
      const aad = fromString.fromString(jwe.aad ? `${jwe.protected}.${jwe.aad}` : jwe.protected, 'utf-8');
      let cleartext = null;
  ...
  function xc20pEncrypter(key) {
    ...
    return (cleartext, aad) => {
      const iv = utils.randomBytes(cipher.nonceLength);
  ...
  function xc20pDirEncrypter(key) {
    const encrypt = function (cleartext, protectedHeader = {}, aad) {
      try {
        ...
        const encodedAad = fromString.fromString(aad ? `${protHeader}.${bytesToBase64url(aad)}` : protHeader, 'utf-8');
